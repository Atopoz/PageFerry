...
Layout spike 提前到 PDF 完整实现之前。否则先接入依赖 Paddle 的 PDF pipeline，再切换 ONNX，只会制造一次确定性的返工——这种顺序没有任何值得维护的浪漫。

## Phase 0：固定 behavior oracle

### 工作

- 以现有文档 runtime、结构回归测试和只读样例作为 behavior oracle。
- 列出 DOCX、PPTX、XLSX、TXT、Markdown、PDF pipeline 的入口、依赖、共享工具和企业耦合点，直接迁核心 pipeline，不凭文件名或旧印象重写。
...
- DOCX、PPTX、XLSX、TXT 与 Markdown 的关键行为都有当前 runtime 测试或 golden corpus 证据。
...
## Phase 2：迁移 DOCX、PPTX、XLSX、TXT 与 Markdown
...
- 迁入 XLSX cell pipeline：跳过公式和结构标识符，合并区域只写主 cell，同步 Excel Table header，并在原子发布前验证 workbook 结构签名。
- DOCX 双语模式复用同一次翻译结果，同时产出标准译文版与段内原文-译文双语版，不为第二个 artifact 重复调用模型。
- 直接迁入当前 TXT 与 Markdown 的读取、分段和回写；Markdown 必须保护代码与链接目标，TXT 必须保留编码和换行风格。
- 为五类轻量格式接入有界 batch fan-out：单 job 默认并发 6，按原 group 顺序写回和推进 progress，不为大文档预建无界 future 队列。
- 在 `backend/modules/` 下分别维护 `plain_text/`、`docx/`、`pptx/`、`xlsx/`，格式专属 runtime 不互相堆叠。
...
- 并发 batch 的完成顺序不改变结构回填、repair/fallback 或 progress 顺序；单 job 上限在 DOCX、PPTX、XLSX、TXT 与 Markdown 上都有确定性测试。
- 取消、异常和进程中止都不覆盖源文件。
...
- 获取并固定可复现的 PP-DocLayoutV3 ONNX artifact、revision、size 与 SHA-256。
- 按官方 export contract 实现 800 × 800 RGB/NCHW 预处理、tensor 读取、label 映射、矩形 overlap filter 与坐标还原。
- 在 macOS arm64 CPU 上比较远程 V2 参考结果和本地 V3 ONNX 输出。
- 记录冷启动、单页延迟、线程数、模型大小与多页批处理策略。

### 当前状态

V3 adapter 已使用 ONNX Runtime `CPUExecutionProvider` 接入，并复用 app-scoped lazy session。单份文档按 `batch_size=1` 顺序推理，跨 job 默认只开放一个 layout slot，单次 inference 默认最多使用 4 个 intra-op thread；不引入 Paddle、GPU 或 CUDA。

D950 前 10 页对照中，共同匹配 layout box 的平均 IoU 为 0.947。V2 在第 9、10 页共出现 74 个 `inline_formula` 误判，V3 未出现这批误判。V3 比 V2 多给出一个 `footer` 只说明该样例的 label 边界不同，不能推出 V3 普遍产生更多 footer，也不能据此加入 page-height 启发式。
