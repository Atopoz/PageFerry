...
历史记录页独占 `GET /api/v1/jobs` 返回的持久化任务列表，展示文件名、语言、provider/model、状态、时间与结果操作。一次任务可以返回多个 artifact；DOCX 双语模式同时显示译文版和双语版，每个 artifact 都能用默认应用打开、在文件夹中定位或调用系统应用选择器。options snapshot 由后端保存并随任务 contract 返回，供后续详情视图复现；列表不把全部选项摊开成说明文字。历史页也不复制上传区、语言选择或 provider 配置表单。
...
模型缺失或 checksum 不符时，PDF 任务在页面光栅化前返回 `pdf_layout_model_missing`。字体目录、单个字体或字体准备失败分别返回稳定 code，并且都发生在删除源文字之前。PageFerry 自有 R2/CDN 与 GitHub Release fallback 已配置；正式发布还需完成有进度、取消、重试和磁盘空间提示的安装 UI。clean-room 验收必须覆盖 app 更新后已安装 pack 仍可复用，以及主源不可达时切换 fallback。
...
- `options_json` 按 `kind` 区分格式并保存补齐默认值后的不可变配置；当前 DOCX 保存 `translate_tables` 与 `bilingual`，PPTX 保存 `translate_tables` 与 `translate_notes`。历史记录必须读取该 snapshot，不能用未来版本的新默认值反推旧任务。
