...
右栏按连接与模型两个紧凑 section 排列：每个 provider 只有一个 API Key，验证成功即进入激活态；模型区提供「同步模型」、enabled 开关和唯一 default 选择。内置 preset 显示当前 effective Base URL，默认值由 catalog 托管，override 放入高级设置而不是普通主表单；字段省略表示保留 override，`null` 或空白表示恢复 catalog 默认值。自定义 provider 创建时输入显示名与 Base URL，首版换地址只能删除后重建，不向 UI 宣称可编辑。discovery 只预览候选；保存时才提交 enabled/default inventory 并执行最小 inference。
...
- `provider_configs` 只保存非敏感连接 metadata、`secret_ref`、`default_model_id`、最近一次 probe 的 effective Base URL snapshot 与可空的 `base_url_override`，并用唯一 `provider_id` 固化“一个 provider 只有一份配置”；override 为空表示跟随 catalog 默认值。`provider_model_configs` 保存 model 来源、enabled/available 与 probe 状态；`custom_provider_definitions` 保存用户定义的显示名与 Base URL。API Key 只写系统 Keychain。
...
| `GET /api/v1/providers`                       | 读取不含 secret 的 provider、effective Base URL、override 状态与 enabled/default model inventory             |
...
| `PUT /api/v1/providers/{id}`                  | 对 enabled models 完成最小 inference 后保存 metadata、inventory、preset Base URL override 与 Keychain secret |
| `DELETE /api/v1/providers/{id}`               | 停用 preset；custom 还会删除其定义，两者都清理 inventory 与 Keychain secret                                  |
...
model discovery 只产生候选列表：有可靠 `models_path` 时返回 `source: "remote"`，否则返回 `source: "catalog"`；它不写 Keychain 或 SQLite。request 可临时携带 frontend 尚未保存的 preset Base URL，省略时继续使用已存 override，`null` 或空白只在本次恢复 catalog 默认值。已保存的 override 会同时驱动后续 discovery、probe 与 translator。`PUT` 必须确认 default model 位于 enabled set，并在所有待启用模型通过最小 inference 后才保存 Key、inventory 与 URL override；失败保留上一次可用配置。
