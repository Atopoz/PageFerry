...
8. `fsync` 临时文件后用 `os.replace` 原子发布标准译文版；任何异常都删除未完成的临时结果。
9. 若创建任务时启用 `bilingual`，复用已经完成的译文版，按页把完整原文页放在左侧、对应译文页放在右侧，并以固定 24pt 间距生成第二个 PDF。输出页数必须与输入一致，不再次调用翻译 provider，也不启用 renderer 的页内双语能力。双语文件同样经过重新打开、语法、页数、几何与 content stream 校验后原子发布；派生失败时同时删除译文版和双语版，使任务 fail-closed。
...
临时文件，校验、`fsync` 后再原子替换。

sidecar lifespan 只记录 `layout` 与 `fonts-common-zh-cn` 的 stat snapshot，不读取约 146.7 MiB
资源计算 checksum，因此已安装资源也不会阻塞 ready handshake。首次 `GET /api/v1/pdf-resources`
或首个 PDF 任务才同步执行完整 SHA-256 校验；校验前的未知 snapshot 一律不报告 ready。通过校验后
缓存 device、inode、size、mtime 与 ctime，后续轮询和 PDF 任务只比较轻量 snapshot，文件变化时才
重新读取完整内容。

缺失时界面列出“PDF 布局检测模型”与“简体中文基础字体”，并显示约 146.7 MiB 总量；只有用户
点击“下载并启用 PDF”才调用受 boot token 保护的安装 endpoint。下载支持进度、取消、失败后原位
重试，并把磁盘不足、网络下载、完整性校验和文件系统失败收敛成稳定错误码。PDF pipeline 在创建
ONNX runtime 前调用同一个资源 gate，未通过时 fail-closed；校验与普通翻译任务都不联网，也不会
隐式触发安装。
...
确定性测试已经覆盖模型输入 tensor、模型缺失、逐页 inference、layout fallback、marker 空值/嵌套/交叉后的 chunk fallback、字体与译文 stream fail-closed、混合扫描页、`Tr 3/7` 和 ExtGState alpha 隐藏的 OCR、图片 signature、源文件不变、原子落盘，以及 PDF 双语版的页数、左右位置、异常清理与 artifact contract。D950 英译中端到端实验验证了 10 页标准译文可重新打开，页面几何与内嵌图片保持不变；拼接式双语输出还需作为同一 golden corpus 的发布验收项持续保留。

进入可分发版本前仍需完成：

- 扩充不同来源的 PDF golden corpus，覆盖旋转页面、竖排文字、复杂表格、字体缺失、坐标溢出和混合语言。
- 在真实升级路径验证 PDF resource pack 复用，并固定旧版本垃圾回收策略。
