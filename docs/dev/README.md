...
- 当前可用格式：DOCX、PPTX、XLSX、TXT、Markdown；界面和文件选择器不展示尚未接通的 PDF。
...
- Pipeline 并发：DOCX、PPTX、XLSX、TXT、Markdown 使用单 job 有界滑动窗口，worker 完整收敛 provider、repair 与 fallback 后再按原 group 顺序 commit/progress；retry 必须先释放共享 slot 再 backoff。
...
- Module：`backend/modules/plain_text/`、`docx/`、`pptx/`、`xlsx/` 独立维护；DOCX 双语版是同一次翻译的派生 artifact，PPTX speaker notes 保持独立结构验收。
