...
from .pdfinterp import Color, PDFGraphicState, TextBlockRecorder
...
class LTChar(LTComponent, LTText):
    ...
    def __init__(
        ...
        source_text_block: Optional[TextBlockRecorder] = None,
    ) -> None:
        ...
        self._source_text_block = source_text_block
    ...
    def __repr__(self) -> str:
        return (
            f"<{self.__class__.__name__} {bbox2str(self.bbox)} matrix={matrix2str(self.matrix)} "
            f"font={self.fontname!r} adv={self.adv} text={self.get_text()!r} "
            f"operator={self.source_operator!r}>"
        )
    ...
    @property
    def source_operator(self) -> Optional[str]:
        if self._source_text_block is None:
            return None
        return self._source_text_block.as_string()

    def get_source_operator(self) -> Optional[str]:
        return self.source_operator

    @property
    def source_text_block(self) -> Optional[TextBlockRecorder]:
        return self._source_text_block

    def get_source_text_block(self) -> Optional[TextBlockRecorder]:
        return self.source_text_block
...
class LTTextLineSource(NamedTuple):
    text_block: TextBlockRecorder
    start: int
    end: int
...
class LTTextBoxSource(NamedTuple):
    text_block: TextBlockRecorder
    start: int
    end: int
...
class LTTextLine(LTTextContainer[TextLineElement]):
    ...
    def __init__(self, word_margin: float) -> None:
        super().__init__()
        self.word_margin = word_margin
        self._char_cursor: int = 0
        self._source_segments: List[LTTextLineSource] = []

    def __repr__(self) -> str:
        return f"<{self.__class__.__name__} {bbox2str(self.bbox)} {self.get_text()!r}>"

    # Incompatible override: Allow LTChar/LTAnno, not only LTComponent.
    def add(self, obj: TextLineElement) -> None:  # type: ignore[override]
        if isinstance(obj, LTChar):
            self._register_char_source(obj)
        super().add(obj)
    ...
    def iter_source_ranges(self) -> Iterator[LTTextLineSource]:
        return iter(self._source_segments)

    def get_source_ranges(self, unique: bool = False) -> List[LTTextLineSource]:
        if unique:
            return self._get_unique_source_ranges()
        return list(self.iter_source_ranges())

    def _register_char_source(self, char: LTChar) -> None:
        index = self._char_cursor
        self._char_cursor += 1
        recorder = char.source_text_block
        if recorder is None:
            return
        if self._source_segments and self._source_segments[-1].text_block is recorder:
            last = self._source_segments[-1]
            self._source_segments[-1] = LTTextLineSource(recorder, last.start, index + 1)
            return
        self._source_segments.append(LTTextLineSource(recorder, index, index + 1))

    def _get_unique_source_ranges(self) -> List[LTTextLineSource]:
        if not self._source_segments:
            return []
        merged: Dict[int, LTTextLineSource] = {}
        order: List[int] = []
        for segment in self._source_segments:
            key = id(segment.text_block)
            if key not in merged:
                merged[key] = segment
                order.append(key)
                continue
            existing = merged[key]
            merged[key] = LTTextLineSource(
                existing.text_block,
                min(existing.start, segment.start),
                max(existing.end, segment.end),
            )
...
class LTTextBox(LTTextContainer[LTTextLine]):
    ...
    def __init__(self) -> None:
        ...
        self.index: int = -1
        self._source_segments: List[LTTextBoxSource] = []
        self._char_cursor: int = 0
    ...
    # Incompatible override: LTTextContainer expects LTTextLine items.
    def add(self, obj: LTTextLine) -> None:  # type: ignore[override]
        super().add(obj)
        self._register_line_sources(obj)

    def iter_source_ranges(self) -> Iterator[LTTextBoxSource]:
        return iter(self._source_segments)

    def get_source_ranges(self, unique: bool = False) -> List[LTTextBoxSource]:
        if unique:
            return self._get_unique_source_ranges()
        return list(self.iter_source_ranges())

    def _register_line_sources(self, line: LTTextLine) -> None:
        segments = line.get_source_ranges()
        char_count = sum(1 for obj in line if isinstance(obj, LTChar))
        if segments:
        ...
        self._char_cursor += char_count

    def _get_unique_source_ranges(self) -> List[LTTextBoxSource]:
        if not self._source_segments:
            return []
        merged: Dict[int, LTTextBoxSource] = {}
        order: List[int] = []
        for segment in self._source_segments:
            key = id(segment.text_block)
            if key not in merged:
